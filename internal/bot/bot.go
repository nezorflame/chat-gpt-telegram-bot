...
func New(cfg *viper.Viper, log *slog.Logger, logLevel slog.Level, chatGPT ChatGPT, db *bolt.DB) (*bot, error) {
	if cfg == nil {
	...
	if chatGPT == nil {
		return nil, errors.New("empty OpenAI client")
	}
	...
	return &bot{api: api, chatGPT: chatGPT, db: db, cfg: cfg, log: log}, nil
}
	...
	for {
		...
		case u := <-updates:
			...
			case strings.EqualFold(u.Message.Command(), b.cfg.GetString("commands.start")):
				...
				fallthrough
			case strings.EqualFold(u.Message.Command(), b.cfg.GetString("commands.new")):
				b.log.With("user_id", u.Message.From.ID).Debug("Got /new command")
				go b.newChat(u.Message)
			case strings.EqualFold(u.Message.Command(), b.cfg.GetString("commands.help")):
				...
				go b.help(u.Message)
			default:
				b.log.With("user_id", u.Message.From.ID).Debug("Got a message - sending it to ChatGPT")
